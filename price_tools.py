...
def openX(fileName):
    ...
    if typeX.lower() == 'xlsx':
        book = openpyxl.load_workbook(filename=fileName,
                                      read_only=False,
                                      keep_vba=False,
                                      data_only=False)                 # xlsx
    else:
        book = xlrd.open_workbook(fileName.encode('cp1251'),
                                  formatting_info=True)                # xls
    return book
...
def currencyType(sheet, rowx, colx):
    '''
    Функция анализирует "формат ячейки" таблицы excel, является ли
    он "денежным" и какая валюта указана в этом формате.
